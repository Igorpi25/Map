...
public class Map {
	...
	public static final float UPDATE_MIN_DISTANCE=3.0f;
  ...
  //--------------Start Chat service-------------------- 
  	...
  	public static void startMapService(Context context){
  		...
  		try {
  			intent = new Intent(context,Class.forName(Communicator.getCommunicatorServiceClass()));
  			intent.putExtra("userid", Session.getUserId());
  	    	Log.d(TAG,"startMapService userid="+Session.getUserId());
  	    	context.startService(intent);
  		} catch (ClassNotFoundException e) {
  			Log.e(TAG, "startMapService ClassNotFoundException e="+e);
  		}
